...
for (const l of pack.lessons)
  for (const b of l.beats) {
    if (b.audio && b.text) add(b.audio, b.text);
    for (const o of b.options ?? []) add(o.echoAudio, o.echo); // what Capy says back when a prayer choice is picked
  }
...
for (const s of pack.stories ?? []) {
  for (const pg of s.pages) add(pg.audio, pg.text);
  add(s.moralAudio, s.moral);
}
