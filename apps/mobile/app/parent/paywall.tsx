...
import { Alert, Image, ImageBackground, Linking, Pressable, ScrollView, StyleSheet, Text, View, type ImageSourcePropType } from "react-native";
...
import { PURCHASES_SANDBOX, PurchaseCancelled, loadPrices, restorePurchases, startTrial, type Plan, type PlanPrice } from "@/entitlements/purchase";
import { track } from "@/backend/events";
import { LEGAL } from "@/parent/legal";
...
export default function Paywall() {
  ...
  const [prices, setPrices] = useState<Partial<Record<Plan, PlanPrice>> | null>(null);
  useEffect(() => { void track("paywall_view", { sandbox: PURCHASES_SANDBOX }); void loadPrices().then(setPrices); }, []);
  const annualPrice = prices?.annual?.price ?? copy.annualPrice, monthlyPrice = prices?.monthly?.price ?? copy.monthlyPrice;
  const annualNote = prices?.annual?.perMonth ? copy.annualNoteLive(prices.annual.perMonth) : copy.annualNote;
  const billing = plan === "annual" ? copy.ctaSubLive(annualPrice, copy.yearWord) : copy.ctaSubLive(monthlyPrice, copy.monthWord);
  ...
  const purchase = async () => {
    ...
    catch (e) { if (!(e instanceof PurchaseCancelled)) Alert.alert(copy.errorTitle, copy.errorBody); }
  ...
  return <View style={[s.root, { paddingTop: insets.top }]} testID="paywall">
    ...
    <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false} testID="paywall-scroll">
      ...
      <View accessibilityRole="radiogroup" accessibilityLabel={copy.choosePlan} style={s.plans}>
        <PlanOption id="annual" selected={plan === "annual"} disabled={busy} onPress={() => pick("annual")} title={copy.annualTitle} price={annualPrice} period={copy.annualPeriod} note={annualNote} badge={copy.bestValue} />
        <PlanOption id="monthly" selected={plan === "monthly"} disabled={busy} onPress={() => pick("monthly")} title={copy.monthlyTitle} price={monthlyPrice} period={copy.monthlyPeriod} note={copy.monthlyNote} />
      </View>
      ...
      <View style={s.details}>
        ...
        {details && <View style={s.timeline}>{copy.timeline.map((step,i) => <View key={step.title} style={s.step}><View style={s.stepIcon}><Icon name={step.icon} size={22} /></View><View style={s.stepText}><Text style={s.stepTitle}>{step.title}</Text><Text style={s.small}>{i === 2 ? billing + ". " + step.body : step.body}</Text></View></View>)}</View>}
      </View>
      <Text style={s.trust}>{copy.trust}</Text>
      <Text style={s.legalNote}>{copy.autoRenew}</Text>
      <View style={s.legalRow}>
        ...
        <Pressable accessibilityRole="link" onPress={() => void Linking.openURL(LEGAL.termsUrl)} style={s.restore}><Text style={s.restoreText}>{copy.terms}</Text></Pressable>
        <Pressable accessibilityRole="link" onPress={() => void Linking.openURL(LEGAL.privacyUrl)} style={s.restore}><Text style={s.restoreText}>{copy.privacy}</Text></Pressable>
      </View>
    </ScrollView>
    <View style={[s.footer, { paddingBottom: Math.max(insets.bottom, 10) }]}>
      <Text style={s.billing} testID="paywall-billing">{billing}</Text>
