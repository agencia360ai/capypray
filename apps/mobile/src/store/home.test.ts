...
describe("the home opens up one door at a time", () => {
  ...
  it("opens the path and the stories after the first lesson, feelings after the second, places after the third", () => {
    expect(ids(homeSections(pack!, { beacons: 0, completed: done("w1d1") }))).toEqual(["today", "bedtime", "journey", "stories"]);
  ...
  it("shows everything when a pack has no home plan", () => {
    ...
    expect(homeSections(bare, { beacons: 0, completed: {} })).toHaveLength(9);
  });
});
...
describe("what Capy announces", () => {
  it("says nothing on the very first visit, then one door per visit", () => {
    ...
    expect(ids(freshSections(pack!, s, {}))).toEqual(["today", "bedtime", "journey", "stories"]);
    ...
    expect(pendingReveal(pack!, s, { ...seen, journey: true, stories: true })).toBeUndefined();
  });
  it("skips a fresh section that has no line to say", () => {
    const s = { beacons: 0, completed: done("w1d1", "w1d2") };
    const seen = { today: true, bedtime: true, journey: true, stories: true, feelings: true } as const;
