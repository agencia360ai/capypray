...
The catalogue contains 447 pack recordings and 8 parent-zone recordings (455 Juan files); the newest thirteen are the opening — Capy's seven lines before the first prayer — and the six lines he says when a door opens on the home. A September 20 audit found 19 spoken strings that were not linked — the lantern reward line, ten story titles and eight place titles; they were recorded in Juan's voice on 21 September and the queue is empty. Nothing kid-facing falls back to device speech any more. See [the audit](voice-audit.md) and its [machine-readable queue](voice-recording-queue.json), which the narration test holds to whatever `--missing` reports. Parent-zone narration remains listed separately by `CAPY_LINES`.
